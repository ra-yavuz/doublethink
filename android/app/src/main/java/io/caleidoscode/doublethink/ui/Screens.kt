                ...
                items(topics, key = { it.id }) { topic ->
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Column(modifier = Modifier.padding(12.dp)) {
                            ...
                            Text(
                                "${when (topic.mode) {
                                    TopicMode.ENCRYPTED -> "Encrypted"
                                    TopicMode.PLAINTEXT -> "Plaintext"
                                    TopicMode.SEALED -> "Sealed inbox"
                                }} · ${topic.channelId}",
...
fun AddTopicScreen(
    ...
    onCreateSealed: (name: String, server: String, channel: String) -> Unit,
    onCancel: () -> Unit,
    statusText: String?,
    modifier: Modifier = Modifier,
) {
    var mode by remember { mutableStateOf(TopicMode.ENCRYPTED) }
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            FilterChip(selected = mode == TopicMode.ENCRYPTED, onClick = { mode = TopicMode.ENCRYPTED }, label = { Text("Encrypted") })
            FilterChip(selected = mode == TopicMode.PLAINTEXT, onClick = { mode = TopicMode.PLAINTEXT }, label = { Text("Plaintext") })
            FilterChip(selected = mode == TopicMode.SEALED, onClick = { mode = TopicMode.SEALED }, label = { Text("Sealed inbox") })
        }
        ...
        when (mode) {
            ...
            TopicMode.SEALED -> {
                Text("A sealed inbox is receive-only. Anyone can send to it using your public key " +
                    "(see About), but only this device can read the messages. Good for a contact form.",
                    style = androidx.compose.material3.MaterialTheme.typography.bodySmall)
            }
            TopicMode.PLAINTEXT -> {
                Text("A plaintext topic is open and unencrypted, ntfy-style.",
                    style = androidx.compose.material3.MaterialTheme.typography.bodySmall)
            }
        }
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Button(
                onClick = {
                    when (mode) {
                        TopicMode.ENCRYPTED -> onCreateEncrypted(name, server.trim(), channel.trim(), secret.trim(), role)
                        TopicMode.PLAINTEXT -> onCreatePlaintext(name, server.trim(), channel.trim())
                        TopicMode.SEALED -> onCreateSealed(name, server.trim(), channel.trim())
                    }
                },
                enabled = channel.isNotBlank() && (mode != TopicMode.ENCRYPTED || secret.isNotBlank()),
            ) { Text("Add") }
        }
    }
}
